...
public class ChainBetting {
    @XmlTransient
    private static Logger log = Logger.getLogger(ChainBetting.class);
    @XmlTransient
    ...
    ChainBetting(String matchLiveName, double betMargin) {
    ...
    void addBet(Bet bet, Betting betting) {
        boolean newBet = true;
    ...
    Bet getNextBet(SelectionBet selectionBet, OddEven oddEven, double minBet, BetMaker betMaker) {
        ...
        Double betPrice = calcBet(minBet, margin, countBet,betMaker);
    ...
    private Double calcBet(double minBet, double margin, int countBet, BetMaker betMaker) {
        Double currentPrice = 0.0;

        double price = calcBetForMatchLive(minBet, margin, countBet);
        double balance = betMaker.getStorage().getCurrentBalance();
        if (price < balance) {
            currentPrice = price;
        } else {
           if (Sender.getSender() != null) {
                if (Sender.getSender().isSwitchOn()) {
                    String message = "У вас не достаточно средств на счету что бы сделать ставку. " +
                            "Сейчас на вашем счету " + balance + " долларов. " +
    ...
    private static Double calcBetForMatchLive(double minBet, double margin, int countBet) {
        Double bet;
        if (countBet == 0) {
            bet = margin / 0.83;
        } else {
    ...
    public double getLastBetPriceInMatch(double minBet) {
        lastQuarterBetPrice = calcBetForMatchLive(minBet, margin, countBet + 3);
        return lastQuarterBetPrice;
    }

    public double getTotalPriceOfBetsInMatch(double minBet) {
        double res = 0.0;
    ...
    public void setMatchLiveName(String matchLiveName, Betting betting) {
        this.matchLiveName = matchLiveName;
        betting.getChainHolder().saveChains();
    }
